...
<script>
     ...
     (function () {
        ...
        $(document).on('keyup','.dt-pups-male',function(){
            let tr = $(this).closest('tr'); 
            let female = isNaN(parseInt(tr.find('.dt-pups-female').val()))? 0 :parseInt(tr.find('.dt-pups-female').val());
            let male = isNaN(parseInt($(this).val())) ? 0 :parseInt($(this).val());
        ...
        $(document).on('keyup','.dt-pups-female',function(){
            let tr = $(this).closest('tr'); 
            let male = isNaN(parseInt(tr.find('.dt-pups-male').val()))? 0 :parseInt(tr.find('.dt-pups-male').val());
            let female = isNaN(parseInt($(this).val())) ? 0 :parseInt($(this).val());
            let total = female+male;
            let pups = parseInt(tr.find('.dt-pups').val());
            let dead_pups = pups - total;
            if(dead_pups < 0){
        ...
        $(document).on('click','#weaning_submit',function (){
            ...
            let delivery_id=[];
            let remarks=[];
            $('.dt-pups-male').each(function(index,item){
                pups_male.push($(item).val());
            });
            $('.dt-pups-female').each(function(index,item){
                pups_female.push($(item).val());
            });
            ...
            formData.append('pups_male', JSON.stringify(pups_male));
            formData.append('pups_female', JSON.stringify(pups_female));
        ...
        var table = $('.datatables-basic').DataTable({
            ...
            function update_action(){
                setTimeout(() => {
                    ...
                    if (addDelivery) {
                        addDelivery.addEventListener('click', function (e) {
                            axios
                    ...
                    if (addWeaning) {
                        addWeaning.addEventListener('click', function (e) {
                            axios
                            .get('/breeding-weaning/'+e.currentTarget.dataset.id)
                            .then(res=>{
                                ...
                                for (const item of res.data.deliveries) {
                                    ...
                                    if(item.weaning){
                                        let weaned_female = isNaN(parseInt(item.weaning.weaned_female)) ? 0 :parseInt(item.weaning.weaned_female);
                                        let weaned_male = isNaN(parseInt(item.weaning.weaned_male)) ? 0 :parseInt(item.weaning.weaned_male);
                                        let total_weaned = weaned_female + weaned_male;
                                        cloneRecord.find('.dt-pups-male').val(item.weaning.weaned_male);
                                        cloneRecord.find('.dt-pups-male').prop('disabled',true);
                                        cloneRecord.find('.dt-pups-female').val(item.weaning.weaned_female);
                                        cloneRecord.find('.dt-pups-female').prop('disabled',true);
                                        cloneRecord.find('.dt-weaned-date').val(item.weaning.date_of_weaned);
                                        cloneRecord.find('.dt-weaned-date').prop('disabled',true);
                                        cloneRecord.find('.dt-weaned-remarks').val(item.weaning.remarks);
                    ...
                    if (showSummary) {
                        showSummary.addEventListener('click', function (e) {
                            axios
                            .get('/breeding-summary/'+e.currentTarget.dataset.id)
                            .then(res=>{
                                t_dead = 0;
                                t_weaned = 0;
                                t_w_female = 0;
                                ...
                                for (const item of res.data.deliveries) {
                                    ...
                                    if(item.weaning){
                                        let weaned_female = isNaN(parseInt(item.weaning.weaned_female)) ? 0 :parseInt(item.weaning.weaned_female);
                                        t_w_female += weaned_female;
                                        let weaned_male = isNaN(parseInt(item.weaning.weaned_male)) ? 0 :parseInt(item.weaning.weaned_male);
                                        t_w_male += weaned_male;
                                        let total_weaned = weaned_female + weaned_male;
                                        cloneRecord.find('.dt-pups-male').html(item.weaning.weaned_male);
                                        cloneRecord.find('.dt-pups-female').html(item.weaning.weaned_female);
                                        cloneRecord.find('.dt-weaned-remarks').html(item.weaning.remarks);
                                ...
                                if(res.data.deliveries){
                                    ...
                                    .append($('<td>').append(t_w_male))
                                    .append($('<td>').append(t_w_female))
                                    .append($('<td>').append(''))
                                    .append($('<td>').append(''))
                                    .append($('<td>').append(''))
                                    .append($('<td>').append(''))
                ...
                <table class="dt-multilingual table border-top" id="add_weaning">
                  <thead>
                    <tr class="table-active">
                      ...
                      <th>Cage No.</th>
                      <th>Delivered Female(s)</th>
                      <th>Pups Born (in Stock)</th>
                      <th>Pups Died Before Weaning</th>
                      <th>Pups Weaned</th>
                      <th>M</th>
                      <th>F</th>
                  ...
                  <tbody>
                    <tr>
                        ...
                        <td>
                            <textarea  name="remakrs[]" class="form-control dt-weaned-remarks"></textarea>
                        </td>
                    </tr>
                  </tbody>
                </table>
      ...
      <div class="modal-content p-3 p-md-5" style="width:1400px;">
        <div class="modal-body">
          ...
          <div id="validation-errors" style="position:relative">
            ...
            <div class="card-datatable table-responsive">
                <table class="dt-multilingual table border-top" id="show_weaning">
                  <thead>
                    <tr class="table-info">
                      ...
                      <th rowspan="2">M</th>
                      <th rowspan="2">F</th>
                      <th colspan="3">Issuable Stock</th>
                  ...
                  <tbody>
                    <tr>
                        ...
                        <td> <span class="dt-pups-male-issuable"></span></td>
                        <td> <span class="dt-pups-female-issuable"></span></td>
                        <td>
